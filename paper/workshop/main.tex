...
%   - The k=4 sequence result         -> two sentences, Section 3.6
...
\begin{table}[t]
  \centering
  \input{\wmfroot tables/tab_encoder}
...
\subsection{The same shape along a sequence}
\label{sec:results:sequence}

Two tasks are a thin reading of ``continual''. We reran MiniGrid as a four-task
sequence, and forgetting of the first task peaks at an intermediate stage there
too rather than growing with sequence position --- the same shape the distance
axis showed, on a different axis.
...
recurrent latent world model, so the results speak to that class. The budget
