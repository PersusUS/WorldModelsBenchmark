...
one pair of extra trainings per cell and seed rather than two: 45 pairs against
the grid's 225 runs, a 20\% surcharge.
...
\begin{table}[t]
  ...
  $d_{\mathrm{trans}}$ is the median over five seeds and is comparable within a
  family, not across families.}
  \label{tab:tasks}
\end{table}
...
\begin{itemize}
  ...
  frozen column into the new one.
  ...
  MC-dropout estimate of epistemic uncertainty \citep{gal2016dropout}, and the
  VAE is frozen after task~A. It is included as one of five methods, not as the
  paper's result.
\end{itemize}
