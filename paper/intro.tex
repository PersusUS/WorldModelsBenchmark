...
plus 45 from-scratch reference pairs --- and this paper is mostly about what it
told us that we had not asked. Both findings are negative, and neither is about
which method wins.
...
is uninformative ($\rho = 0.05$) while the difficulty of the new task predicts
forgetting far better ($\rho = 0.58$), which is why we read the result as
forgetting scaling with what the new task demands of the model rather than with
...
its prediction fidelity reads $-6.21$ --- in the frozen basis, the model
...
the transition component almost perfectly ($-0.07$) while its encoder degrades
by $753$. A benchmark reporting only PF and RD would certify that neither model
forgot anything.
...
\begin{enumerate}
  ...
  aggregation we tried and at two training budgets. We recommend reporting a
...
single aggregate score, because the one we started from is $78$--$97\%$ rollout
divergence by weight. It does not report a policy-impact metric, which an
