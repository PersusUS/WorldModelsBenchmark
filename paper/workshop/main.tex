...
%
% A SEPARATE document, not a variant of ../main.tex: at 8 pages the full
% paper's 12,300 words do not survive as a subset, so this was rewritten rather
% than cut. What follows the long version exactly is the numbers -- the tables
% are the same generated files, pulled from ../tables/, and every figure in the
% prose is one that appears in them or was verified against results/ during the
% number pass.
%
...
%
% Compile FROM paper/, not from this directory, so that bibtex can open
% refs.bib -- it refuses paths that climb out of the compile directory, and
% \wmfroot below cannot help it because bibtex does not expand LaTeX macros:
%
%   cd paper && pdflatex workshop/main && bibtex workshop/main \
%     && pdflatex workshop/main && pdflatex workshop/main
%
% Overleaf already compiles from the project root, so uploading paper/ and
% setting workshop/main.tex as the main document is the same thing.
...
% Where paper/ sits, seen from wherever LaTeX was started. Compiled locally in
% this directory it is one level up; compiled by Overleaf, which runs from the
% project root with this file as the main document, it is the current
% directory. Probing for a file that only exists under paper/ settles it, so
% the same source builds either way and nothing has to be duplicated here.
\IfFileExists{tables/tab_axis.tex}{\def\wmfroot{}}{\def\wmfroot{../}}

\graphicspath{{\wmfroot figures/}{figures/}{../figures/}}
...
\begin{table}[t]
  \centering
  \input{\wmfroot tables/tab_predictors}
...
The blind spot that Section~\ref{sec:benchmark} declared is large. Held-out
task-A reconstruction error, read as a factor of the same model's error before
the switch, runs from $1.00$ --- nothing lost --- to $848$ across the grid; the
full table is in the released results. Fine-tuning's grows by a factor of $811$
in MiniGrid at minimum distance while its PF for that cell is $-5.58$: measured
in the frozen latent basis, the model \emph{improved}. This is not an isolated
sign error --- fine-tuning's PF is negative in eight of the nine cells while its
encoder is destroyed in six.
...
% Not \wmfroot here. The prefix is for LaTeX, which expands macros; this name
% reaches bibtex through the .aux file, and bibtex refuses to open a path that
% climbs out of the compile directory. Compiling from paper/ -- which is what
% Overleaf does, and what the header tells a local build to do -- makes the
% bare name correct and leaves the .bib single-sourced.
\bibliography{refs}
